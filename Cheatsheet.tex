...
\usepackage[margin=0.5cm]{geometry} % Narrow margins to fit more content
...
\textbf{Closed Form}: $(X^TX + \alpha I)^{-1}X^Ty$ \\
...
\textbf{w}: $\text{sign}(w)\max(0, |w_{LS}| - \alpha)$ \\
...
\textbf{Usage}: Avoid overfitting, Obtain numerically more stable solutions, enforce the desired parameter space as a prior
}
...
\Block{%
\textbf{How to Perform PCA}\\
Given data matrix $X\in\mathbb R^{n\times d}$ (rows = samples, columns = features).\\
1.\ \textbf{Center (and scale) data}: compute mean $\mu=\frac1n\sum_i x_i$, set $\tilde x_i=x_i-\mu$; optionally standardize each feature to unit variance.\\
2.\ \textbf{Compute covariance or use SVD}: covariance $S=\tfrac1n\tilde X^\top\tilde X$ and solve eigenproblem $S w_k=\lambda_k w_k$; or compute SVD $\tilde X=U\Sigma V^\top$ and take columns of $V$ as principal directions.\\
3.\ \textbf{Sort components}: sort eigenvalues (or singular values) descending; choose top $k$ such that $\sum_{j=1}^k\lambda_j/\sum_{j=1}^d\lambda_j$ reaches desired variance (e.g.\ 95\%).\\
4.\ \textbf{Project data}: form $W_k=[w_1,\dots,w_k]$ and compute low-dim representation $Z=\tilde X W_k\in\mathbb R^{n\times k}$ (rows are principal component scores).\\
5.\ \textbf{(Optional) Reconstruct}: approximate original data by $\hat X=Z W_k^\top+\mu$ for visualization or compression error analysis.\\
\textbf{Notes:} Always center data; scaling is crucial if features have different units. Use truncated/randomized SVD for large, sparse, or high-dimensional $X$.
}
...
\textbf{Healthy curve}: high-loss start, rapid decreases, gradually flattens as model converges \\ 
\textbf{Underfitting}: Decrease slightly, flat at hight loss\\
\textbf{Large learning rate, small batch, noisy data}: Decrease with oscillation\\
\textbf{Too small learning rate, bad initialization}: Initially flat then drop\\
\textbf{Overfitting}: Good decreasing for training, val/test loss drop then increase\\
